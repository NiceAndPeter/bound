...
namespace
{
  using cell  = bnd::bound<{0, 100, 0.5}>;
  using accum = bnd::bound<{0, 1000, 0.5}>;
...
TEST_CASE("algo: ranges algorithms over vector<bound>", "[algo][ranges]")
{
  ...
  REQUIRE(rng::all_of(v,  [](cell x){ return x > 0;   }));
  REQUIRE(rng::any_of(v,  [](cell x){ return x == 10; }));
  REQUIRE(rng::none_of(v, [](cell x){ return x > 100; }));
}
