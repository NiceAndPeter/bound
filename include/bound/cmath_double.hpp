...
namespace bnd::math::dbl::detail
{
  ...
  inline double d_cos(double x) { return d_sin(x + (kHalfPiHi + kHalfPiLo)); }
  ...
  inline double d_sqrt(double x) { return std::sqrt(x); }   // correctly rounded
  ...
  inline double d_tan(double x)   { return d_sin(x) / d_cos(x); }
  inline double d_exp2(double x)  { return d_exp(x * kLn2Full); }
  inline double d_log2(double x)  { return d_log(x) * kLog2e; }
  inline double d_log10(double x) { return d_log(x) * kLog10e; }
  inline double d_pow(double b, double e) { return d_exp(e * d_log(b)); }
  inline double d_cbrt(double x)
  {
    if (x == 0.0) return 0.0;
    double m = d_exp(d_log(x < 0 ? -x : x) * (1.0 / 3.0));
    return x < 0 ? -m : m;
  }
  inline double d_sinh(double x) { double e = d_exp(x); return (e - 1.0 / e) * 0.5; }
  inline double d_cosh(double x) { double e = d_exp(x); return (e + 1.0 / e) * 0.5; }
  ...
  inline double d_atan2(double y, double x)
  {
  ...
  inline double d_asin(double x) { return d_atan(x / d_sqrt((1.0 - x) * (1.0 + x))); }
  inline double d_acos(double x) { return kPiHalf - d_asin(x); }
} // namespace bnd::math::dbl::detail
